...
package struct SearchResultActionRouter {
    ...
    package func route(_ action: SearchResultAction, item: DescendantSearchItem, root: URL, canAccess: (URL) -> Bool, itemExists: Bool) -> SearchResultActionRoute {
        guard canAccess(root), canAccess(item.url), FilePathComparison.isSameOrDescendant(item.url, ofDirectory: root), itemExists else { return .unavailable }
