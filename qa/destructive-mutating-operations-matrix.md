...
| MUT-031 | Cloud-only iCloud item | A disposable iCloud Drive item made cloud-only by optimized storage, if available | Attempt copy/move before downloading; download it in Finder and retry. | Cloud-only item is rejected before mutation with download/retry guidance; the downloaded item follows normal operation handling. | `FileOperationService` availability preflight. |
| MUT-032 | Mounted network share | Disposable writable SMB/NFS share | Browse, copy, move, rename, and trash a disposable file; disconnect the share and retry one operation. | Mounted/reachable share works subject to macOS permissions; disconnect reports volume unavailable and makes no new mutation. | Access policy, availability preflight, partial-result reporting. |
| MUT-033 | Read-only/ejected removable media | Disposable disk image or empty external volume | Attempt a copy to a read-only mount, then eject during/retry an operation. | Read-only target is rejected with writable-media guidance; ejected target/source is rejected with reconnect/remount guidance. | Writable/availability preflight. |
| MUT-034 | Metadata preservation and warning | Disposable file with permissions, timestamp, tag, xattr, and ACL where supported | Copy locally and to a network/removable fixture; inspect destination before removing source. | Supported metadata is retained; unsupported metadata produces a cleanup warning instead of a silent full-success claim. | `preserveMetadata`, cleanup warnings. |
| MUT-035 | Finder alias rejected | Finder alias to a disposable target | Attempt copy, move, rename, trash, and permanent delete. | Each mutation is rejected before changing the alias or target, with Finder/original-item recovery guidance. | Finder-alias preflight. |
